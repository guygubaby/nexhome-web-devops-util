...
var require_dayjs_min = __commonJS({
  "node_modules/dayjs/dayjs.min.js"(exports, module2) {
    !function(t, e) {
      typeof exports == "object" && typeof module2 != "undefined" ? module2.exports = e() : typeof define == "function" && define.amd ? define(e) : t.dayjs = e();
    }(exports, function() {
      "use strict";
...
// src/index.ts
__export(exports, {
  DOCKER_IMAGE_NAME_LABEL: () => DOCKER_IMAGE_NAME_LABEL,
  DOCKER_IMAGE_PREFIX: () => DOCKER_IMAGE_PREFIX,
  ENV_FILE_NAME: () => ENV_FILE_NAME,
  ENV_LABEL: () => ENV_LABEL,
  bootstrap: () => bootstrap,
  generateDockerComposeStr: () => generateDockerComposeStr,
  generateDockerTag: () => generateDockerTag,
  getEnvName: () => getEnvName,
  getTimeStamp: () => getTimeStamp,
  log: () => log,
  loginAndPushDockerImage: () => loginAndPushDockerImage,
  packAndBuildDockerImage: () => packAndBuildDockerImage,
  repeatDash: () => repeatDash,
  runAppLocally: () => runLocally2,
...
var ENV_LABEL = "MY_WEB_BUILD_ENV";
var ENV_FILE_NAME = "env-file";
var DOCKER_IMAGE_NAME_LABEL = "DOCKER_IMAGE_NAME_LABEL";
var DOCKER_IMAGE_PREFIX = "registry.cn-shanghai.aliyuncs.com/nexhome/";

// src/utils/others.ts
var import_child_process = __toModule(require("child_process"));
var import_dayjs = __toModule(require_dayjs_min());
var import_process = __toModule(require("process"));
var import_qiniu = __toModule(require("qiniu"));
var repeatDash = (count = 10) => {
  return "-".repeat(count) + ">" || "->";
};
var setEnv = (envName) => {
  import_process.default.env[ENV_LABEL] = envName;
};
var unSetEnv = () => {
  delete import_process.default.env[ENV_LABEL];
};
var getEnvName = () => {
  const envName = import_process.default.env[ENV_LABEL];
  if (!envName)
    throw new Error(`${ENV_LABEL} is null ,\u8BF7\u8BBE\u7F6E ${ENV_LABEL}`);
  return envName;
};
var runShell = (command) => {
  return new Promise((resolve, reject) => {
    const executedCommand = import_child_process.default.spawn(command, {
      stdio: "inherit",
      shell: true
    });
...
var getTimeStamp = () => {
  return (0, import_dayjs.default)(new Date()).format("YYYYMMDD");
};
var generateDockerComposeStr = (dockerImageName, deployPort, containerName) => {
  return `
  version: '3'
  services:
    web-service:
...
var deleteFile = async (bucket, remoteFileName, config, mac) => {
  return new Promise((resolve, reject) => {
    const bucketManager = new import_qiniu.default.rs.BucketManager(mac, config);
...
var uploadToQiniu = async (dockerComposeStr, qiniuAccountInfo, remoteFileName) => {
  return new Promise(async (resolve, reject) => {
    ...
    const putPolicy = new import_qiniu.default.rs.PutPolicy(options);
    const config = new import_qiniu.default.conf.Config();
    config.zone = import_qiniu.default.zone.Zone_z2;
...
// src/utils/index.ts
var generateDockerTag = (pkg, cicdType = "jenkins") => {
  const { version } = pkg;
  const prop = cicdType === "jenkins" ? "GIT_COMMIT" : "CI_COMMIT_SHORT_SHA";
  const { [prop]: sha } = process.env;
  const tags = [version, getTimeStamp(), (sha || "noopnoop").substr(0, 8)];
  const tag = tags.join("-");
  return tag;
};
var runLocally = (port, dockerImageName, dockerContainerName) => {
  const scripts = `
...
var packAndBuildDockerImage = (dockerImageName, dockerFileName, packCommand = "yarn build") => {
  const scripts = `
...
var loginAndPushDockerImage = (account, dockerImageName) => {
...
var bootstrap = async (config) => {
  ...
  const dcStr = generateDockerComposeStr(dockerImageName, deployedPort, appName);
...
var runLocally2 = async (config) => {
  ...
  log(dockerImageName);
  await packAndBuildDockerImage(dockerImageName, dockerfile, buildCommand);
  await runLocally(deployedPort, dockerImageName, containerName);
};
// Annotate the CommonJS export names for ESM import in node:
0 && (module.exports = {
  DOCKER_IMAGE_NAME_LABEL,
  DOCKER_IMAGE_PREFIX,
  ENV_FILE_NAME,
  ENV_LABEL,
  bootstrap,
  generateDockerComposeStr,
  generateDockerTag,
  getEnvName,
  getTimeStamp,
  log,
